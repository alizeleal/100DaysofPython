...
def checkResources(coffee):
    ...
    if resources["water"] >= MENU[f"{coffee}"]["ingredients"]["water"] & resources["coffee"] >= \
            MENU[f"{coffee}"]["ingredients"]["coffee"]:
        if "milk" in MENU[f"{coffee}"]:
            if resources["milk"] >= MENU[f"{coffee}"]["ingredients"]["milk"]:
                return True
            else:
...
def countMoney(coffee):
    ...
    totalInserted = quartersInserted * coins["quarter"] + dimesInserted * coins["dime"] + nicklesInserted * coins[
        "nickel"] + penniesInserted * coins["penny"]
